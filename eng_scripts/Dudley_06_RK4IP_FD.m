...
%FWHM = 50*1e-3;            %szerokoœæ po³ówkowa impulsu
%T0 = FWHM/(2*log(1+sqrt(2)));
...
v_c = c/lambda_centralna;                    %czêstotliwoœæ centralna [2*pi*THz]

Ld = (T0^2)/(abs(Beta(2)));                  %droga dyspersji [km]
Ln = 1/(P0 * gamma);                         %droga nieliniowoœci [km]

z = 0.00015;                                 %d³ugoœæ œwiat³owodu [km]
dz = 1e-7;                                   %krok przestrzenny [km]
...
while propagedlength < z,
    ...
    %nan_region = find(isnan(Af(ii,:)));
    %Af(ii,nan_region) = 1e-100;
...
%print('-f1','Dudley_3D_rk4ip_1001_FD','-dpng')
...
colorbar
set(gcf,'renderer','zbuffer');
%print('-f2','Dudley_time_rk4ip_1001_FD','-dpng')
...
caxis([-40, 0]); xlim([430 1300])
shading interp; 
xlabel('\lambda [nm]'); ylabel('z [m]');
colorbar
set(gcf,'renderer','zbuffer');
%print('-f3','Dudley_lambda_rk4ip_1001_FD','-dpng')
